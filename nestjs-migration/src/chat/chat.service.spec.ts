...
describe("ChatService", () => {
	...
	describe("addRoom", () => {
		...
		it("방 save 중 에러 발생 시 반환", async () => {
			mockRoomRepository.save.mockRejectedValue(
				new Error("방 생성 실패")
			);

			await expect(
				chatService.addRoom(mockCreateRoomDto)
		...
		it("멤버 save 중 에러 발생 시 반환", async () => {
			...
			mockMemberRepository.save.mockRejectedValue(
				new Error("멤버 생성 실패")
			);

			await expect(
				chatService.addRoom(mockCreateRoomDto)
			).rejects.toThrow("멤버 생성 실패");
	...
	describe("getRoomsBykeyword", () => {
		...
		it("DB에서 키워드로 채팅방 목록을 불러오기 성공", async () => {
			mockRoomRepository.findAndCountBy.mockResolvedValue([
				"",
				mockTotalRoom,
			]);
			...
			const result =
				await chatService.getRoomsBykeyword(readRoomByKeywordDto);
			...
			expect(roomRepository.findAndCountBy).toHaveBeenCalledWith({
				name: Like(`%${readRoomByKeywordDto.keyword.trim()}%`),
			});
			expect(roomRepository.getRoomByKeyword).toHaveBeenCalledWith(
		...
		it("DB에서 키워드로 채팅방 목록 불러오는 중 에러 발생 시 반환", async () => {
			...
			mockRoomRepository.getRoomByKeyword.mockRejectedValue(
				new Error("mock error")
			);

			await expect(
				chatService.getRoomsBykeyword(readRoomByKeywordDto)
	...
	describe("getRoomsByUserId", () => {
		...
		it("DB에서 유저 아이디로 채팅방 목록을 불러오기 성공", async () => {
			mockMemberRepository.getRoomCntByUserId.mockResolvedValue(
				mockTotalRoom
			);
			...
			const result =
				await chatService.getRoomsByUserId(readRoomByUserIdDto);
			...
			expect(memberRepository.getRoomCntByUserId).toHaveBeenCalledWith(
				readRoomByUserIdDto.userId
			);
			expect(roomRepository.getRoomByUserId).toHaveBeenCalledWith(
				readRoomByUserIdDto
			);
		});
		it("DB에서 유저 아이디로 채팅방 목록 불러오는 중 에러 발생 시 반환", async () => {
			...
			mockRoomRepository.getRoomByUserId.mockRejectedValue(
				new Error("mock error")
			);

			await expect(
				chatService.getRoomsByUserId(readRoomByUserIdDto)
			).rejects.toThrow(new Error("mock error"));
	...
	describe("getMessageLogs", () => {
		it("메세지 로그 불러오기 성공", async () => {
			...
			expect(roomRepository.getMessageLogs).toHaveBeenCalledWith(
				mockRoomId
			);
		});
		it("메세지 로그 불러오는 중 에러 발생 시 반환", async () => {
			mockRoomRepository.getMessageLogs.mockRejectedValue(
				new Error("mock error")
			);

			await expect(
				chatService.getMessageLogs(mockRoomId)
			).rejects.toThrow(new Error("mock error"));
			expect(roomRepository.getMessageLogs).toHaveBeenCalledWith(
				mockRoomId
			);
	...
	describe("addUserToRoom", () => {
		...
		it("채팅방에 유저를 추가하는 도중 에러 발생 시 반환", async () => {
			...
			await expect(
				chatService.addUserToRoom(mockJoinRoomDto)
			).rejects.toThrow(new Error("mock error"));
	...
	describe("enterUserToRoom", () => {
		...
		it("채팅방에 들어가는 도중 에러 발생 시 반환", async () => {
			mockMemberRepository.findOne.mockRejectedValue(
				new Error("mock error")
			);

			await expect(
				chatService.enterUserToRoom(mockEnterRoomDto)
			).rejects.toThrow(new Error("mock error"));
			expect(memberRepository.findOne).toHaveBeenCalledWith({
				where: {
					user: { id: mockUser.userId },
	...
	describe("leaveRoom", () => {
		...
		it("채팅방에 나가기 성공", async () => {
			...
			expect(memberRepository.update).toHaveBeenCalledWith(
				{
					user: { id: mockLeaveroomDto.userId },
					room: { id: mockLeaveroomDto.roomId },
				},
				{ isDeleted: true }
			);
		});
		it("채팅방에 나가는 도중 에러 발생 시 반환", async () => {
			mockMemberRepository.update.mockRejectedValue(
				new Error("mock error")
			);

			await expect(
				chatService.leaveRoom(mockLeaveroomDto)
			).rejects.toThrow(new Error("mock error"));
			expect(memberRepository.update).toHaveBeenCalledWith(
				{
					user: { id: mockLeaveroomDto.userId },
