...
describe('ChatController', () => {
  ...
  describe("POST /room", () => {
    ...
    it("채팅방 생성 성공", async () => {
      ...
      const result = await chatController.handleRoomCreate(mockUser, mockCreateRoomBodyDto);

      expect(chatService.addRoom).toHaveBeenCalledWith(
        {...mockCreateRoomBodyDto, userId: mockUser.userId}
      );
  ...
  describe("GET /rooms", () => {
    describe("키워드 채팅방 목록 불러오기", () => {
      ...
      it("키워드를 이용해 채팅방 목록 불러오기 성공", async() => {
        ...
        const result = await chatController.handleRoomsRead(mockReadRoomQueryDto,mockUser);
  
        expect(chatService.getRoomsBykeyword).toHaveBeenCalledWith(mockReadRoomQueryDto);
      ...
      it("키워드를 이용해 채팅방 목록 불러오기 실패", async() => {
        mockChatService.getRoomsBykeyword.mockRejectedValue(new Error('mock error'));
  
        await expect(
          chatController.handleRoomsRead( mockReadRoomQueryDto,mockUser),
        ).rejects.toThrow(ServerError.reference("채팅방 불러오기 실패"));
    ...
    describe("사용자 아이디 채팅방 목록 불러오기", () => {
      ...
      it("USER_ID를 이용해 채팅방 목록 불러오기 성공", async() => {
        ...
        expect(chatService.getRoomsByUserId).toHaveBeenCalledWith(readRoomByUserIdDto);
      ...
      it("USER_ID를 이용해 채팅방 목록 불러오기 실패", async() => {
        mockChatService.getRoomsByUserId.mockRejectedValue(new Error('mock error'));
  ...
  describe("GET /room/:room_id", () => {
    it("메세지 로그 불러오기 성공", async () => {
      ...
      const result = await chatController.handleMessageLogsRead(mockRoomId);
    ...
    it("메세지 로그 불러오기 실패", async() => {
      mockChatService.getMessageLogs.mockRejectedValue(new Error("mock error"));
  ...
  describe("POST /join", () => {
    ...
    it("채팅방 가입 성공", async() => {
      ...
      const result = await chatController.handleRoomJoin(mockJoinRoomBodyDto, mockUser);

      expect(result).toEqual(
        {roomId: mockRoomId}
      )
      expect(chatService.addUserToRoom).toHaveBeenCalledWith(mockJoinRoomDto);
    });
    it("채팅방 가입 실패", async () => {
      mockChatService.addUserToRoom.mockRejectedValue(new Error("mock error"));
  ...
  describe("POST /enter", () => {
    ...
    it("채팅방 입장 성공", async() => {
      ...
      const result = await chatController.handleRoomEnter(mockEnterRoomBodyDto, mockUser);
      ...
      expect(chatService.enterUserToRoom).toHaveBeenCalledWith(mockEnterRoomDto);

    });
    it("채팅방 입장 실패", async() => {
      mockChatService.enterUserToRoom.mockRejectedValue(new Error("mock error"));
      ...
      await expect(chatController.handleRoomEnter(mockEnterRoomBodyDto, mockUser)).rejects.toThrow("채팅방 입장 실패");
  ...
  describe("POST /leave", () => {
    ...
    it("채팅방 나가기 성공", async() => {
      ...
      const result = await chatController.handleRoomLeave(mockLeaveRoomBodyDto, mockUser);
      ...
      expect(chatService.leaveRoom).toHaveBeenCalledWith(mockLeaveRoomDto);

    });
    it("채팅방 나가기 실패", async () => {
      mockChatService.leaveRoom.mockRejectedValue(new Error("mock error"));
      ...
      await expect(chatController.handleRoomLeave(mockLeaveRoomBodyDto, mockUser)).rejects.toThrow("채팅방 나가기 실패");
