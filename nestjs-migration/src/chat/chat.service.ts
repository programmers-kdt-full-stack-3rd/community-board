...
export class ChatService {
  ...
  async getRoomsBykeyword (readRoomByKeywordDto: ReadRoomByKeywordDto): Promise<GetRoomsResultDto> {
      const { keyword } = readRoomByKeywordDto;

      try {  
      const result = await this.roomRepository.findAndCountBy({ name: Like(`%${keyword.trim()}%`)});
      const totalRoomCount = result[1];

      const dataRows = await this.roomRepository.getRoomByKeyword(readRoomByKeywordDto);
  ...
  async getRoomsByUserId (readRoomByUserIdDto: ReadRoomByUserIdDto) : Promise<GetRoomsResultDto> {
    const {userId} = readRoomByUserIdDto;

    try {
      const totalRoomCount = await this.memberRepository.getRoomCntByUserId(userId);

      const dataRows = await this.roomRepository.getRoomByUserId(readRoomByUserIdDto);
  ...
  async leaveRoom(leaveRoomDto: LeaveRoomDto) : Promise<boolean> {
    ...
    try {
      const result = await this.memberRepository.update({user: {id: userId}, room: {id: roomId}}, {isDeleted: true});
